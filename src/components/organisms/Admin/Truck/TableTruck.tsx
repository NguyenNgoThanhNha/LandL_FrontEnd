...
import { Table, TableBody, TableCell, TableHeader, TableRow, TableHead } from '@/components/atoms/ui/table'
              ...
              {filteredTrucks.length > 0 ? (
                filteredTrucks.map((truck) => (
                  <TableRow key={truck.truckId}>
                    ...
                    <TableCell>
                      <div
                        className={`rounded-full flex justify-center text-white hover:bg-opacity-75 ${
                          truck.status === 'Active' ? 'bg-green-400 hover:bg-green-600' : 'bg-red-400 hover:bg-red-600'
                        }`}
                      >
                        {truck.status}
                      </div>
